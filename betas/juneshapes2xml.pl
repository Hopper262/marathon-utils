...
#     my $file;
#     open $file, '>', 'unpacked_256_' . $id . '.rsrc' or die;
#     print $file $data;
#     close $file;
    
    push(@collinfo, [ $coll, length($data), $coll, 8, $data, $colltype ]);
  }
  elsif ($name eq 'sgrp')
  {
    my $ct = ReadUint16();
...
{
  my (undef, $len, $coll, $depth, $data, $colltype) = @$cref;
#   next if $coll > 3;
  SetReadSource($data);
  ...
  my $lcount2 = ($boff - $loff) / 32;
  warn "Low count mismatch in $coll: $lcount2 vs. $lcount\n" if ($lcount != $lcount2);
