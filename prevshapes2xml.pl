...
print STDERR <<END;
Data: offset $dataOff, size $dataSize
Map:  offset $mapOff, size $mapSize
END
...
for my $ti (1..$numTypes)
{
  ...
  print STDERR "Found type: $typename\n";
...
for my $tref (sort { $a->[0] <=> $b->[0] } @typeinfo)
{
  ...
  {
    my $id = ReadUint16();
    print STDERR "Found item: $name $id\n";
    ReadPadding(2);
...
for my $dref (sort { $a->[0] <=> $b->[0] } @datainfo)
{
  ...
  print STDERR "Adding collection $coll at offset $off\n";
}
